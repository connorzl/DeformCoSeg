...
for GV in GV_all:
    ...
    lat_params = torch.nn.Parameter(torch.randn(GV.shape[0], NUM_PARTS), requires_grad=True).to(device)
    lat_params_all.append(lat_params)

# Flow layer.
func = NeuralFlowDeformer(adjoint=False, dim=3, latent_size=256 + NUM_PARTS, out=3, device=device)
func.add_lat_params(lat_params_all)
func.to(device)
...
for it in range(int(args.num_iter)):
    ...
    for i, (src, targ) in enumerate(deformation_pairs):
        ...
        res = func(GV_device_all[src], source_target_latents[i], src)
        src_part_masks = func.get_lat_params(src) 

        flow = torch.zeros(src_part_masks.shape[0], 3).to(device)
        part_mask_sum = torch.zeros(src_part_masks.shape[0], 1).to(device)
        for i in range(NUM_PARTS):
            part_mask_i = src_part_masks[:, i].unsqueeze(1)
            flow += part_mask_i * (res - GV_device_all[src])
            part_mask_sum += part_mask_i
        GV_deformed = GV_device_all[src] + torch.div(flow, part_mask_sum)

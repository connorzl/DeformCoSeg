...
def sample_faces(vertices, faces, n_samples=10**4):
  ...
  # Need to loop to exactly sample n_samples, since np.random.choice can return duplicate samples.
  # We could set replace=False, but then we would require the number of faces containing 
  # at least 1 sample to be greater than the number of samples we need to remove.
  # Worst case: one huge face containing most of the samples: O(n_samples).
  """
  while True:
    ...
    unique, counts = np.unique(floor_indices, return_counts=True)

    # Faces with few samples might hit negative number of samples.
    n_samples_per_face[unique] -= counts
    n_samples_per_face[n_samples_per_face < 0] = 0
    floor_num = np.sum(n_samples_per_face) - n_samples
  """
  floor_num = np.sum(n_samples_per_face) - n_samples
  if floor_num > 0:
